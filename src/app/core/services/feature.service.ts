...
const MOCK_FEATURES: Feature[] = [
  {
    id: "feature1",
    name: "Export Data",
    description: "Ability to export data in CSV, and Excel formats",
...
export class FeatureService {
  ...
  private orgFeaturesSubject = new BehaviorSubject<OrganizationFeature[]>(
    this.orgFeatures
  );
  ...
  getOrganizationFeatures(
    ...
    { feature: Feature; isEnabled: boolean; canToggle: boolean }[]
  > {
      ...
      map((org) => {
        return this.features.map((feature) => {
          const orgFeature = this.orgFeatures.find(
            (of) =>
              of.organizationId === organizationId &&
              of.featureId === feature.id
          );
  ...
  toggleFeature(
    ...
    featureId: string,
    isEnabled: boolean
  ): Observable<OrganizationFeature> {
    ...
    if (index === -1) {
      ...
      this.toastService.show({
        type: "success",
        message: `Feature "${feature?.name}" has been ${
  ...
  checkFeatureEligibility(
    organizationId: string,
    featureId: string
  ): Observable<{ eligible: boolean; reason?: string }> {
    return this.organizationService.getOrganization(organizationId).pipe(
      map((org) => {
        const feature = this.features.find((f) => f.id === featureId);
