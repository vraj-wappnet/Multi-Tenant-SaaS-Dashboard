        ...
        <div class="page-title">
          ...
          <p class="text-muted">
            @if (isSuperAdmin && currentOrg) {
              Manage feature toggles for {{ currentOrg.name }}
            } @else if (isSuperAdmin) {
              Select an organization to manage its features
            } @else {
              Manage feature toggles for your organization
            }
          </p>
        </div>
      </div>
                  ...
                  <tr [class.disabled-feature]="!item.canToggle">
                    ...
                    <td>
                      <div class="toggle-container">
                        <div class="feature-toggle" [class.disabled]="!item.canToggle">
                          <input 
                            type="checkbox" 
                            [id]="'feature-' + item.feature.id" 
                            [checked]="item.isEnabled"
                            [disabled]="!item.canToggle || toggling[item.feature.id]"
                            (change)="toggleFeature(item.feature, $event)">
                          <label 
                            [for]="'feature-' + item.feature.id" 
                            [title]="!item.canToggle ? 'Not available on ' + currentOrg?.plan + ' plan' : ''">
                        ...
                        @if (toggling[item.feature.id]) {
                          ...
                          <span class="toggle-status">{{ item.isEnabled ? 'Enabled' : 'Disabled' }}</span>
                        }
                        @if (!item.canToggle) {
                          <div class="toggle-tooltip">
                            Not available on {{ currentOrg?.plan }} plan. Upgrade to 
                            @if (currentOrg?.plan === 'FREE' && item.feature.availableOnPlans.includes('PRO')) {
                              PRO
                            } @else {
                              ENTERPRISE
                            } 
...
export class FeatureTogglesComponent {
  ...
  toggleFeature(feature: Feature, event: Event) {
    ...
    this.featureService.toggleFeature(this.organizationId, feature.id, isEnabled).subscribe({
      next: () => {
